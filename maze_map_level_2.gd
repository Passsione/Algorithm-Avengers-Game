...
const ROWS = 25    
const COLS = 25
const WALL = Vector2i(8, 4)
const PATH = Vector2i(9, 4)
...
var maze_change_interval = 2.0 # How often difficulty increases
var wall_shift_timer = 0.0
var wall_shift_interval = 7.0
var shifting_walls = []
...
func initialize_maze():
	...
	generate_maze()
	game_timer = 0.0 
	time_limit = max(1.0, 90 - (difficulty_level * 5.0))
	player_dead = false 
...
func shift_random_walls():
	...
	for _i in range(walls_to_shift):
		...
		tilemaplayer.set_cell(0, Vector2i(wall_pos.y, wall_pos.x), 0, PATH)
...
func _restore_shifted_walls(walls: Array):
	for wall_pos in walls:
		...
		# Update TileMap visually
		tilemaplayer.set_cell(0, Vector2i(wall_pos.y, wall_pos.x), 0, WALL)
...
func draw_maze():
	...
	for r in range(ROWS):
		for c in range(COLS):
			...
			tilemaplayer.set_cell(0, Vector2i(c, r), 0, tile) # Set cell on TileMap

	# Redraw exit in case it was overwritten during drawing
	if exit_position:
		tilemaplayer.set_cell(0, exit_position, 0, EXIT)
